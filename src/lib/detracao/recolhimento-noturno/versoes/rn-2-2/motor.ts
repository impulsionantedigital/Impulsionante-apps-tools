...
import { feriadosDoIntervalo, feriadosNacionais, ROTULO_DIA_SEMANA_POR_EXTENSO } from './feriados'
...
function classeDoDia(
  ...
  // Feriado declarado à mão pelo membro (municipal/estadual) — vale 24h sem depender do checkbox.
  if (segmento.feriadosIntegral.some((f) => f.data === dataISO)) return 'feriadoIntegral'
...
export function calcular(entrada: EntradaCalculo): ResultadoCalculo {
  ...
  // 🔴 A lista do que a tela MOSTRA: os nacionais (quando o checkbox está ligado) MAIS os
  // declarados à mão (municipais e estaduais) — que antes ficavam de fora e por isso o resumo
  // dizia "6 feriados" enquanto o membro havia informado 8. Cada um diz de onde veio, porque no
  // documento a diferença importa: um é lei federal, o outro é decisão da comarca.
  const feriadosConsiderados: FeriadoConsiderado[] = [
    ...entrada.segmentos
      .filter((s) => s.incluirFeriadosUteis)
      .flatMap((s) => feriadosDoIntervalo(s.dataInicio, s.dataFim))
      .map((f) => ({ ...f, origem: 'nacional' as const })),
    ...entrada.segmentos
      .flatMap((s) => s.feriadosIntegral)
      // Data em branco é linha que o membro ainda não preencheu — não é feriado nenhum, e listá-la
      // como se fosse poria uma linha vazia no anexo.
      .filter((f) => /^\d{4}-\d{2}-\d{2}$/.test(f.data))
      .map((f) => ({
        data: f.data,
        nome: f.nome?.trim() || 'Feriado local',
        // 🔴 O mesmo rótulo por extenso dos feriados nacionais (`lista.ts`): os dois se leem lado a
        // lado na mesma lista, e "Quarta" ao lado de "quarta-feira" seria inconsistência à toa.
        //
        // ⚠️ A função recebe a DATA, não o `Weekday`: `diaSemanaDe` devolve `'WED'`, e passar isso
        // aqui dava `new Date('WEDT00:00:00Z')` → `NaN` → `undefined`. A chave existia no objeto e
        // o valor não, que é o tipo de defeito que passa despercebido numa checagem de forma.
        diaSemana: ROTULO_DIA_SEMANA_POR_EXTENSO(f.data),
