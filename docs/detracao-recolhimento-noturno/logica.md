...
| Versão | Data | Mudança |
|--------|------|---------|
| **RN-2.1** | 2026-10 | 🔴 **O CÁLCULO NÃO MUDOU.** A precedência (folga integral → feriado → regra noturna), a contagem por dia, a conversão em anos/meses/dias e a lista nacional de feriados são **idênticas** às da RN-2.0 — há teste que compara as duas versão a versão (`rn21-modo-simples.spec.ts`). O que mudou é a **TELA**: o modo simples passou a ser o único. Saíram o toggle "avançado", os múltiplos segmentos, o fuso configurável, o monitoramento eletrônico e as observações — os três últimos eram metadados de auditoria que o motor nunca leu. Os **feriados municipais e estaduais** (`feriadosIntegral`) subiram para o formulário único: o campo já era lido pelo motor desde a RN-2.0, e o que faltava era um jeito de preenchê-lo sem esconder-se atrás do modo avançado. A RN-2.0 fica no registro, congelada, para os cálculos gravados nela continuarem abrindo. |
