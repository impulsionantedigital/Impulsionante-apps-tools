...
if len(sys.argv) != 2 or sys.argv[1] != '2025':
    # Só existe planilha de 2025. Um decreto novo traz planilha nova, com células
    # próprias: o mapeamento abaixo NÃO serve para outro ano sem revisão.
    sys.exit('uso: python validacao/oraculo.py 2025')
...
APOIO = ['P6', 'P9', 'P13', 'P14', 'P16', 'P17', 'F148', 'F149']
...
def main():
    ...
    for idx, sc in enumerate(cenarios):
        ...
        apoio = {cel: valor(sol, c(cel)) for cel in APOIO}
        saida.append({'_nome': nome, 'entrada': sc, 'planilha': planilha, 'apoio': apoio})
        print('ok  %s' % nome, file=sys.stderr)

    doc = {
        '_leia': (
            'GERADO por validacao/oraculo.py a partir de validacao/2025/planilha.xlsx. '
